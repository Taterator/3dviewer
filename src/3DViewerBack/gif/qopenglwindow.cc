...
/*!
  ...
  \inmodule QtGui
  \since 5.4
  \brief The QOpenGLWindow class is a convenience subclass of QWindow to perform
  OpenGL painting. QOpenGLWindow is an enhanced QWindow that allows easily
  creating windows that perform OpenGL rendering using an API that is compatible
  with QOpenGLWidget and is similar to the legacy QGLWidget. Unlike
  QOpenGLWidget, QOpenGLWindow has no dependency on the widgets module and
  offers better performance. A typical application will subclass QOpenGLWindow
  and reimplement the following virtual functions: \list \li initializeGL() to
  perform OpenGL resource initialization \li resizeGL() to set up the
  transformation matrices and other window size dependent resources \li
  paintGL() to issue OpenGL commands or draw using QPainter \endlist To schedule
  a repaint, call the update() function. Note that this will not immediately
  result in a call to paintGL(). Calling update() multiple times in a row will
  not change the behavior in any way. This is a slot so it can be connected to a
  \l QTimer::timeout() signal to perform animation. Note however that in the
  modern OpenGL world it is a much better choice to rely on synchronization to
  the vertical refresh rate of the display. See
  \l{QSurfaceFormat::setSwapInterval()}{setSwapInterval()} on a description of
  the swap interval. With a swap interval of \c 1, which is the case on most
  systems by default, the \l{QOpenGLContext::swapBuffers()}{swapBuffers()} call,
  that is executed internally by QOpenGLWindow after each repaint, will block
  and wait for vsync. This means that whenever the swap is done, an update can
  be scheduled again by calling update(), without relying on timers. To request
  a specific configuration for the context, use setFormat() like for any other
  QWindow. This allows, among others, requesting a given OpenGL version and
  profile, or enabling depth and stencil buffers. Unlike QWindow, QOpenGLWindow
  ...
  legacy QGLWidget. In contrast, \c PartialUpdateBlit and \c PartialUpdateBlend
  are more in line with QOpenGLWidget's way of working, where there is always an
  ...
  Qt::AA_ShareOpenGLContexts attribute. When enabled, the OpenGL contexts of all
  QOpenGLWindow instances will share with each other. This allows accessing each
  other's shareable OpenGL resources. For more information on graphics in Qt,
  see \l {Graphics}.
 */
...
/*!
  ...
  default framebuffer after each paint. This allows having QPainter-based
  drawing code in paintGL() which only repaints a smaller area at a time,
  because, unlike NoPartialUpdate, the previous content is preserved. \value
  PartialUpdateBlend Similar to PartialUpdateBlit, but instead of using
...
class QOpenGLWindowPrivate : public QPaintDeviceWindowPrivate {
 ...
 public:
  QOpenGLWindowPrivate(QOpenGLContext *shareContext,
                       QOpenGLWindow::UpdateBehavior updateBehavior)
      : updateBehavior(updateBehavior),
        hasFboBlit(false),
        shareContext(shareContext) {
    if (!shareContext) this->shareContext = qt_gl_global_share_context();
  }
...
void QOpenGLWindowPrivate::beginPaint(const QRegion &region) {
    ...
    if (!fbo || fbo->size() != deviceSize) {
      ...
      if (samples > 0) {
        if (updateBehavior != QOpenGLWindow::PartialUpdateBlend)
          ...
          qWarning(
              "QOpenGLWindow: PartialUpdateBlend does not support "
              "multisampling");
      }
  ...
  if (updateBehavior > QOpenGLWindow::NoPartialUpdate) fbo->bind();
}

void QOpenGLWindowPrivate::endPaint() {
  Q_Q(QOpenGLWindow);

  if (updateBehavior > QOpenGLWindow::NoPartialUpdate) fbo->release();
  ...
  if (updateBehavior == QOpenGLWindow::PartialUpdateBlit && hasFboBlit) {
    ...
    extensions.glBindFramebuffer(GL_DRAW_FRAMEBUFFER,
                                 context->defaultFramebufferObject());
    extensions.glBlitFramebuffer(0, 0, deviceWidth, deviceHeight, 0, 0,
                                 deviceWidth, deviceHeight, GL_COLOR_BUFFER_BIT,
                                 GL_NEAREST);
    ...
    QMatrix4x4 target =
        QOpenGLTextureBlitter::targetTransform(windowRect, windowRect);
    blitter.bind();
    blitter.blit(fbo->texture(), target,
 ...
 */
QOpenGLWindow::QOpenGLWindow(QOpenGLWindow::UpdateBehavior updateBehavior,
                             QWindow *parent)
    : QPaintDeviceWindow(*(new QOpenGLWindowPrivate(nullptr, updateBehavior)),
                         parent) {
...
/*!
  Constructs a new QOpenGLWindow with the given \a parent and \a updateBehavior.
  The QOpenGLWindow's context will share with \a shareContext. \sa
...
/*!
  ...
  \since 5.5
*/
QOpenGLWindow::~QOpenGLWindow() { makeCurrent(); }
...
/*!
  ...
  advanced, multi-threaded scenarios where a thread different than the GUI or
  main thread may want to update the surface or framebuffer contents. See
  QOpenGLContext for more information on threading related issues. This function
  is suitable for calling also when the underlying platform window is already
  destroyed. This means that it is safe to call this function from a
  QOpenGLWindow subclass' destructor. If there is no native window anymore, an
  offscreen surface is used instead. This ensures that OpenGL resource cleanup
  operations in the destructor will always work, as long as this function is
  called first. \sa QOpenGLContext, context(), paintGL(), doneCurrent()
 */
...
/*!
  \return The QOpenGLContext requested to be shared with this window's
  QOpenGLContext.
*/
...
QImage QOpenGLWindow::grabFramebuffer() {
  ...
  QImage img =
      qt_gl_read_framebuffer(size() * devicePixelRatio(), hasAlpha, hasAlpha);
...
/*!
  ...
  makeCurrent(). \note Scheduling updates from here is not necessary. The
  windowing systems will send expose events that trigger an update
  automatically. \sa initializeGL(), paintGL()
 */
...
/*!
  ...
  \c PartialUpdateBlit. This mode involves blitting the extra framebuffer used
  by paintGL() onto the default framebuffer after each invocation of paintGL(),
...
int QOpenGLWindow::metric(PaintDeviceMetric metric) const {
  ...
  switch (metric) {
    case PdmDepth:
      if (d->paintDevice) return d->paintDevice->depth();
      break;
    default:
      break;
  }
